...
describe('Function `evaluateField`', () => {
  ...
  it('handles conflicting constraints on the same path', () => {
    ...
    // @ts-expect-error -- `fields` is valid for group type but not single fields
    system.character.attributes.fields.strength.constraints = [
      { if: { this: { $lt: 10 } }, then: { allowed: true, reason: 'Strength must be less than 10' } },
      { if: { this: { $lte: 10 } }, then: { allowed: false, reason: 'Strength cannot be less than or equal to 10' } },
    ];
    ...
    expect(result).toEqual({
      ...
      violations: [
        {
          path: 'attributes.strength',
          reason: 'Strength must be less than 10',
        },
        {
          path: 'attributes.strength',
          reason: 'Strength cannot be less than or equal to 10',
        },
      ],
    });
  });
  ...
  it('returns false if both allow and disallow constraints exist but none match', () => {
    ...
    // @ts-expect-error -- `fields` is valid for group type but not single fields
    system.character.attributes.fields.strength.constraints = [
      { if: { this: { $eq: 5 } }, then: { allowed: true, reason: 'Strength must equal 5' } },
      { if: { this: { $eq: 7 } }, then: { allowed: false, reason: 'Strength cannot equal 7' } },
    ];
